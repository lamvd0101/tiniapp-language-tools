...
import {createScanner} from './txmlScanner';
...
export class Parser {
  ...
  public parseDocument(document: TextDocument): TXMLDocument {
    return this.parse(document.getText());
  }

  public parse(text: string): TXMLDocument {
    const scanner = createScanner(text, undefined, undefined, true);

    const document = new Node(0, text.length, [], void 0);
    ...
    return {
      roots: document.children,
      findNodeBefore: document.findNodeBefore.bind(document),
      findNodeAt: document.findNodeAt.bind(document),
    };
  }
}
